...
def test_folder_1(tmp_path):
    source = SAMPLES / "test_odf.odt"
    document_src = Document(source)
    tmp_source = tmp_path / "test_odf.odt"
    ...
    print(err)
    assert exitcode == 0
    path = tmp_path / "test_odf.odt.folder"
    assert path.is_dir()
...
def test_folder_2(tmp_path):
    source = SAMPLES / "test_odf.odt"
    document_src = Document(source)
    document_src.save(tmp_path / "test_odf.odt")
    tmp_source = tmp_path / "test_odf.odt"
    ...
    params2 = str(tmp_path / "test_odf.odt.folder")
